...
public class BaseReceiver {
    ...
    protected Map<String, MessageReceivedWebhook> message_received_webhook =
            new HashMap<String, MessageReceivedWebhook>();

    protected Map<String, MessageDeliveredWebhook> message_delivered_webhook =
            new HashMap<String, MessageDeliveredWebhook>();

    protected Map<String, MessageEchoWebhook> message_echo_webhook =
            new HashMap<String, MessageEchoWebhook>();

    protected Map<String, MessageReadWebhook> message_read_webhook =
            new HashMap<String, MessageReadWebhook>();

    protected Map<String, PostbackWebhook> postback_webhook =
            new HashMap<String, PostbackWebhook>();
    ...
    public BaseReceiver parse() {
        ...
        if (!this.message_object.has("object")
                || !this.message_object.getString("object").equals("page")) {
            return this;
        }
        ...
        for (i = 0; i < entry.length(); i++) {
            ...
            if (entry_obj.has("messaging")) {
                ...
                for (i = 0; i < entry_obj.getJSONArray("messaging").length(); i++) {
                    ...
                    JSONObject messaging_item =
                            entry_obj.getJSONArray("messaging").getJSONObject(i);
                    ...
                    if (messaging_item.has("message")) {
                        ...
                        if (message.has("is_echo")) {
                            ...
                            // Message
                            // -----------------
                            this.message_received_webhook.put(
                                    "message." + z, new MessageReceivedWebhook());
                            this.message_received_webhook.get("message." + z).setUserId(sender_id);
                            this.message_received_webhook
                                    .get("message." + z)
                                    .setPageId(recipient_id);
                            this.message_received_webhook
                                    .get("message." + z)
                            ...
                            if (message.has("quick_reply")) {
                                ...
                                String payload =
                                        (quick_reply.has("payload"))
                                                ? quick_reply.getString("payload")
                                                : null;
                                this.message_received_webhook
                                        .get("message." + z)
                                        .setQuickReplyPayload(payload);
                            }
                                ...
                                for (i = 0; i < attachments.length(); i++) {
                                    ...
                                    if (type.equals("audio")) {
                                        if (attachment.has("payload")) {
                                            JSONObject attachment_payload =
                                                    attachment.getJSONObject("payload");
                                            String url =
                                                    (attachment_payload.has("url"))
                                                            ? attachment_payload.getString("url")
                                                            : null;

                                            this.message_received_webhook
                                                    .get("message." + z)
                                                    .setAttachment("audio", url);
                                        }
                                        ...
                                        if (attachment.has("payload")) {
                                            JSONObject attachment_payload =
                                                    attachment.getJSONObject("payload");
                                            Long coordinates_lat =
                                                    (attachment_payload.has("coordinates.lat"))
                                                            ? attachment_payload.getLong(
                                                                    "coordinates.lat")
                                                            : null;
                                            Long coordinates_long =
                                                    (attachment_payload.has("coordinates.long"))
                                                            ? attachment_payload.getLong(
                                                                    "coordinates.long")
                                                            : null;
                                            this.message_received_webhook
                    ...
                    if (messaging_item.has("delivery")) {
                        ...
                        this.message_delivered_webhook.put(
                                "message." + z, new MessageDeliveredWebhook());
                        ...
                        if (delivery.has("watermark")) {
                            this.message_delivered_webhook
                                    .get("message." + z)
                                    .setWatermark(delivery.getLong("watermark"));
                        }
                        if (delivery.has("seq")) {
                            this.message_delivered_webhook
                                    .get("message." + z)
                                    .setSeq(delivery.getInt("seq"));
                        }
                    ...
                    if (messaging_item.has("read")) {
                        ...
                        if (read.has("watermark")) {
                            this.message_read_webhook
                                    .get("message." + z)
                                    .setWatermark(read.getLong("watermark"));
                        }
                        if (read.has("seq")) {
                            this.message_read_webhook
                                    .get("message." + z)
                                    .setSeq(read.getInt("seq"));
                        }
                    }
